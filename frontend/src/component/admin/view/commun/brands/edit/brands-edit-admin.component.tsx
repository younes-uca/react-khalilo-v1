...
type BrandsUpdateScreenRouteProp = RouteProp<{ BrandsUpdate: { brands: BrandsDto } }, 'BrandsUpdate'>;

type Props = { route: BrandsUpdateScreenRouteProp; };

const BrandsAdminEdit: React.FC<Props> = ({ route }) => {
    ...
    const { control, handleSubmit } = useForm<BrandsDto>({
        defaultValues: {
            id: brands.id ,
            name: brands.name ,
        },
    ...
    const handleUpdate = async (item: BrandsDto) => {
        Keyboard.dismiss();
...
return(
    <SafeAreaView style={{ flex: 1, backgroundColor: '#e6e8fa' }}>
        ...
        <ScrollView style={{ margin: 20 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
            ...
            <Text style={{ fontSize: 30, fontWeight: 'bold', alignSelf: 'center', marginBottom: 10 }} >Update Brands</Text>

            <CustomInput control={control} name={'name'} placeholder={'Name'} keyboardT="default" />

            <CustomButton onPress={handleSubmit(handleUpdate)} text={"Update Brands"} bgColor={'#ffa500'} fgColor={'white'} />
...
export default BrandsAdminEdit;
