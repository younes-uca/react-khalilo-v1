...
const BrandsAdminCreate = () => {
    ...
    const { control, handleSubmit, reset } = useForm<BrandsDto>({
        defaultValues: {
        name: '' ,
        },
    });
    ...
    const handleSave = async (item: BrandsDto) => {
        Keyboard.dismiss();
        try {
...
return(
    <SafeAreaView style={{ flex: 1, backgroundColor: '#e6e8fa' }} >
        <ScrollView style={{ margin: 20, marginBottom: 80 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" >
            <Text style={{ fontSize: 30, fontWeight: 'bold', alignSelf: 'center', marginBottom: 10 }} >Create Brands</Text>

            <TouchableOpacity onPress={brandsCollapsible} style={{ backgroundColor: 'orange', padding: 10, borderRadius: 10, marginVertical: 5 }}>
        ...
        <CustomButton onPress={handleSubmit(handleSave)} text={"Save Brands"} bgColor={'#000080'} fgColor={'white'} />
        </ScrollView>
...
export default BrandsAdminCreate;
