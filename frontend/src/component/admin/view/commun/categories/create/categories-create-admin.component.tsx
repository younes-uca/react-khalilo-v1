...
const CategoriesAdminCreate = () => {
    ...
    const { control, handleSubmit, reset } = useForm<CategoriesDto>({
        defaultValues: {
        name: '' ,
        sortby: null ,
        categorieId: null ,
    ...
    const handleSave = async (item: CategoriesDto) => {
        Keyboard.dismiss();
        try {
...
return(
    <SafeAreaView style={{ flex: 1, backgroundColor: '#e6e8fa' }} >
        <ScrollView style={{ margin: 20, marginBottom: 80 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" >
            <Text style={{ fontSize: 30, fontWeight: 'bold', alignSelf: 'center', marginBottom: 10 }} >Create Categories</Text>

            <TouchableOpacity onPress={categoriesCollapsible} style={{ backgroundColor: 'orange', padding: 10, borderRadius: 10, marginVertical: 5 }}>
        ...
        <CustomButton onPress={handleSubmit(handleSave)} text={"Save Categories"} bgColor={'#000080'} fgColor={'white'} />
        </ScrollView>
...
export default CategoriesAdminCreate;
